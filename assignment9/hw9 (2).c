// name: Yuanmeng Zhang
// email: zhang.yuanme@northeastern.edu
...
int main () {
  ...
  while (!isEmpty(q)) {
    ...
    /*
     1. need to assign true when a node is done
     2. use queue to implement BFS
     3, isEmpty(queue_t* q_p) funtion, can repeate tree BFS
     */
      int temp = dequeue(q);        //dequeue function returns an int,this will return 0 due to line 226
      done[temp] = true;
      printf("NODE: %d\n", temp);
      for (int i = 0; i < GSIZE; i++){    //not sure if can improve to O(log(n))
          if (E[temp][i] && !done[i]){    //enqueue only once for each node
              enqueue(q,i);
          }
          if (E[temp][i]) {              //once enqueued, label as done
              done[i] = true;
          }
      }
  }
