...
int kruskal(Fedge e[], int n) { 
    ...
    for (i = 0; i < edge_num; ++i) {
        u = find_set(e[i].u);       // find set of u
        v = find_set(e[i].v);       // find set of v
        if (u != v) {               // if u and v are not in the same set
            mst_e += e[i].key;      // add weight to min cost
            ++mst_e_n;              // increment the number of edges
            union_set(u, v);        // merge sets
            if (mst_e_n == n - 1)    // if we have n-1 edges in MST, we are done
                break;
        }
    }
    return mst_e; 
...
int main() {
    ...
    int i, j;
    for (i = 0; i < VSIZE; ++i) {
        make_set(i);                // create sets for each vertex
        for (j = i + 1; j < VSIZE; ++j) {
            if (graph[i][j] != INF) {   // if there is an edge
