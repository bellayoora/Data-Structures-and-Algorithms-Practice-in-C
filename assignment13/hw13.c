...
// Repeat each alphabetical character in the input string based on the next digit's value
while (ptr < ptrend) {
    if (isalpha(*ptr)) {
        int next_digit = getcount(ptr + 1);
        int repeat_count = (next_digit == 0) ? 1 : next_digit;
